    ...
    private static void startToTrackingFromConfig(Path configDir) {
        if (Files.exists(configDir)) {
            ErrorPolicy.DEPURATE_ERROR.executeTaskWithMsg(
                    ...
                    "[Conditional Innate Skills] Error reading Conditional Innate config"
            );
        } else {
        ...
        paths.filter(p -> p.toString().endsWith(".json"))
                .forEach(root ->
                        ErrorPolicy.DEPURATE_ERROR.executeTaskWithMsg(
                                ...
                                "[Conditional Innate Skills] Error reading: " + root
                        )
                )
        ;
    }
            ...
            for (var entry : simpleInnateSkillsBuilders.entrySet()) {
                ...
                for (Path file : entry.getValue()) {
                    ...
                    ErrorPolicy.DEPURATE_ERROR.executeTaskWithMsg(
                            ...
                            "[Conditional Innate Skills] Error reading: " + file
                    );
                }
            }
        } else {
