...
public class ConditionalInnateSkillBuilder {
    private static final Map<String, List<RawConditionalInnateSkillBuilderData>> CONDITIONAL_INNATE_SKILL_BUILD_DATA = new ConcurrentHashMap<>();
    ...
    private static void startToTrackingFromConfig(Path configDir) {
        ...
        ExecutionTasks.operateAndGetResult(
                ...
                ExtendedDatapacks.LOGGER.error("[Conditional Innate Skills] Error reading Conditional Innate config", e)
        );
    }
        ...
        paths.filter(p -> p.toString().endsWith(".json"))
                .forEach(root ->
                        ExecutionTasks.runAndGetResult(
                                ...
                                ExtendedDatapacks.LOGGER.error("[Conditional Innate Skills] Error reading: {}", root, e)
                        )
                )
        ;
    ...
    private static void startToTrackingFromAPI() {
        ...
        for (var entry : simpleInnateSkillsBuilders.entrySet()) {
            ...
            for (Path file : entry.getValue()) {
                ...
                ).ifFailure(e -> ExtendedDatapacks.LOGGER.error("[Conditional Innate Skills] Error reading: {}", file, e));
            }
        }
    }
    ...
    private static void startToLoad(Path file, String modId) throws IOException {
        ...
        ExtendedDatapacks.LOGGER.info(
                "[Conditional Innate Skills] Reading file: {}",
                file.getFileName()
        );
    ...
    private static void startToProcessConditionalInnateSkills(JsonObject root, String fileName, String modId) {
        if (modId.equals("config")) {
            if (!root.has("modid")) {
                ExtendedDatapacks.LOGGER.warn("[Conditional Innate Skills] missing 'modid' in: {}", fileName);
                return;
            }
            modId = root.get("modid").getAsString();
        } else {
            if (root.has("modid")) {
                ExtendedDatapacks.LOGGER.warn("[Conditional Innate Skills] 'modid' ignored in {}, since file is loaded from jar.", fileName);
            }
        }

        if (!root.has("name")) {
            ExtendedDatapacks.LOGGER.warn("[Conditional Innate Skills] Skill without 'name' in {}", fileName);
            return;
        }

        String name = root.get("name").getAsString();
        if (!root.has("conditions")) {
            ExtendedDatapacks.LOGGER.warn("[Conditional Innate Skills] Missing 'conditions' object in {}", fileName);
            return;
        }
        ...
        for (Map.Entry<String, JsonElement> entry : conditionsObj.entrySet()) {
            ...
            if (!conditionBody.has("animation")) {
                ExtendedDatapacks.LOGGER.warn("[Conditional Innate Skills] Condition '{}' missing 'animation' in {}", conditionKey, fileName);
                continue;
            }
            if (!conditionBody.has("properties_type")) {
                ExtendedDatapacks.LOGGER.warn("[Conditional Innate Skills] Condition '{}' missing 'properties_type' in {}", conditionKey, fileName);
                continue;
            }
            if (!conditionBody.has("properties")) {
                ExtendedDatapacks.LOGGER.warn("[Conditional Innate Skills] Condition '{}' missing 'properties' in {}", conditionKey, fileName);
                continue;
            }
            ...
            animations.put(type, new ConditionalAnimationData(animation, phases));
        }
        ...
        CONDITIONAL_INNATE_SKILL_BUILD_DATA.computeIfAbsent(modId, k -> new ArrayList<>()).add(data);
    }
