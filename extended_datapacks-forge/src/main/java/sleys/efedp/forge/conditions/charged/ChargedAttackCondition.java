package sleys.efedp.forge.conditions.charged;
...
public class ChargedAttackCondition extends Condition.EntityPatchCondition {
    ...
    public boolean predicate(LivingEntityPatch<?> entityPatch) {
        if (entityPatch instanceof PlayerPatch<?> playerPatch) {
            ...
            final var isChargedAttack = ChargedAttack.getPressedCastKey(player);
            if (isChargedAttack) {
                boolean sameCategory = mainHandCategory.equals(offHandCategory);
                if (offHandCapability && sameCategory) return true;
