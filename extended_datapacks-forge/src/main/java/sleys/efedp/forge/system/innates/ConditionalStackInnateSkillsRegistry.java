...
import sleys.efedp.forge.system.innates.json.builder.ConditionalStackInnateSkillBuilder;
...
import sleys.efedp.forge.system.innates.json.builder.wrapper.conditional.WConditionalStackInnateSkill;
...
public class ConditionalStackInnateSkillsRegistry {
    private static final List<String> RUNTIME_ERRORS = Collections.synchronizedList(new ArrayList<>());

    private ConditionalStackInnateSkillsRegistry() {}

    @SubscribeEvent
    public static void initialize(SkillBuildEvent build) {
        ExtendedDatapacks.LOGGER.info("[Conditional Stack Innate Skill Registry] Registering JSON skills");

        var data = ConditionalStackInnateSkillBuilder.getStacksConditionalInnateSkillBuildData();
        if (data.isEmpty()) {
            ExtendedDatapacks.LOGGER.info("[Conditional Stack Innate Skill Registry] No JSON skills found");
            return;
        }
    ...
    private static void registerSkill(SkillBuildEvent build, SkillBuildEvent.ModRegistryWorker modRegistry,
                                      String modId, ConditionalStackInnateSkillDefinition skillData) {
        ...
        ExecutionTasks.runAndGetResult(
                ...
                .peekError(exception -> ExtendedDatapacks.LOGGER.fatal(
                        "[Conditional Stack Innate Skill Registry] Error Stack: ", exception
                ))
    ...
    private static void buildSkill(SkillBuildEvent.ModRegistryWorker modRegistry,
        ...
        modRegistry.build(name, WConditionalStackInnateSkill::new,  builder);
    }
    ...
    public static void onClientModBusEvent(final FMLLoadCompleteEvent event) {
        if (!RUNTIME_ERRORS.isEmpty()) {
            throw new RegistryObjectException(
                    "Failure during the operation to create a Conditional Stack Innate Skill...\n" +
