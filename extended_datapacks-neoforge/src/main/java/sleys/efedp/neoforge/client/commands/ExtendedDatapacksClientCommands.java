package sleys.efedp.neoforge.client.commands;
...
public class ExtendedDatapacksClientCommands {
    ...
    private static int getActuallyPassive(CommandContext<CommandSourceStack> context) {
        ...
        if (patchPlayer == null) return 0;
        if (itemCapability.isEmpty()) return 0;
        player.displayClientMessage(Component.literal("Your handed weapon passive is: " + itemCapability.get().getPassiveSkill(patchPlayer)), false);
        return 1;
    }
