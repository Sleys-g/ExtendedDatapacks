...
public class OverlayPacketCoroutine extends CoroutineTask implements IEventListener {
    ...
    private OverlayAssetPacksBuilder.OverlayEffect currentEffect = null;
    private ItemStack lastItem = null;

    public OverlayPacketCoroutine() {
    ...
    private void startToMatching(LocalPlayer player, LocalPlayerPatch playerPatch) {
        var currentItem = player.getWeaponItem();
    ...
    @Nullable
    private OverlayAssetPacksBuilder.OverlayEffect findMatchingEffect(LocalPlayerPatch playerPatch, ItemStack item) {
        var effects = OverlayAssetPacksBuilder.getEffectsForItem(item);
        if (effects.isEmpty()) return null;

        OverlayAssetPacksBuilder.OverlayEffect bestStyleOrSkillEffect = null;
        for (OverlayAssetPacksBuilder.OverlayEffect effect : effects) {
    ...
    private boolean checkConditionStillMet(LocalPlayerPatch playerPatch, ItemStack item, OverlayAssetPacksBuilder.OverlayEffect effect) {
    ...
    private void runOverlay(float screenWidth, float screenHeight, float alpha, ResourceLocation[] resource, int fps,
        ...
        bb.addVertex(0.0F, screenHeight, 0.0F).setUv(0.0F, 1.0F);
        bb.addVertex(screenWidth, screenHeight, 0.0F).setUv(1.0F, 1.0F);
        bb.addVertex(screenWidth, 0.0F, 0.0F).setUv(1.0F, 0.0F);
