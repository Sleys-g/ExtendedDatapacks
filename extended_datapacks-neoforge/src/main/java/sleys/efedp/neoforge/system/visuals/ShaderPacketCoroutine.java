package sleys.efedp.neoforge.system.visuals;
...
import sleys.efedp.neoforge.system.visuals.json.ActivationType;
...
public class ShaderPacketCoroutine extends CoroutineTask {
    ...
    protected boolean run() {
        ...
        if (!PlayerHelper.isValidPlayerAllowDeath(player) || level == null) return true;
    ...
    private void onPlayerClientTick(Player player, PlayerPatch<?> playerPatch) {
        ...
        var dataList = ShaderAssetsPacksBuilder.getForCategory(categoryKey);
    ...
    private void onStylePayload(ShaderAssetsPacksBuilder.ShaderPacket packet, Style style, Item weapon, Set<EffectCondition> validThisTick) {
    ...
    private void onSkillPayload(ShaderAssetsPacksBuilder.ShaderPacket packet, Item weapon, Set<EffectCondition> validThisTick) {
    ...
    private void onAnimationPayload(ShaderAssetsPacksBuilder.ShaderPacket packet) {
    ...
    private String getHoldShaderId(EffectCondition condition, IShaderParameters shader) {
        return switch (condition) {
            case StyleCondition sc -> "HOLD-" + BuiltInRegistries.ITEM.getKey(sc.weapon()) + "-" + sc.style() + "-" + shader.effectType();
            case SkillCondition sk -> "HOLD-" + BuiltInRegistries.ITEM.getKey(sk.weapon()) + "-" + sk.skill() + "-" + shader.effectType();
        };
    }
    ...
    private void dispatchOneshotShader(IShaderParameters shader, float elapse) {
        ...
        if (!isAlreadyActive(shader, id)) {
            switch (shader) {
                case ImpactFrameParams i -> i.sendCTS(id);
                case ColoredImpactFrameParams ci -> ci.sendCTS(id);
                default -> shader.create(id);
            }
        }
    }
