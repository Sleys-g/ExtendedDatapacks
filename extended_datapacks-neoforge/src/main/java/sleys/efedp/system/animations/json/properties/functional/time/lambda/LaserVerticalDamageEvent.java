...
public record LaserVerticalDamageEvent(Float damage,
                                       Optional<Vec3> originOffset) implements IAnimationEventParams {

    public static final MapCodec<LaserVerticalDamageEvent> CODEC = RecordCodecBuilder.mapCodec(instance ->
            instance.group(
                    Codec.FLOAT.fieldOf("damage").forGetter(LaserVerticalDamageEvent::damage),
                    Vec3.CODEC.optionalFieldOf("origin_offset").forGetter(LaserVerticalDamageEvent::originOffset)
            ).apply(instance, LaserVerticalDamageEvent::new)
    );

    @Override
    public <T extends StaticAnimation> void execute(AssetAccessor<T> accessor, LivingEntityPatch<?> patch) {
        var livingCaster = patch.getOriginal();
        if (this.isInvalid(livingCaster.level(), AnimationEvent.Side.BOTH, "Laser Vertical Event")) return;
        ...
        if (level.isClientSide) {
            ...
            var points = LaserEventHelper.computeGroundedLazerPath(level, casterPos, casterPos, 1.3, 3.0, 32.0);
