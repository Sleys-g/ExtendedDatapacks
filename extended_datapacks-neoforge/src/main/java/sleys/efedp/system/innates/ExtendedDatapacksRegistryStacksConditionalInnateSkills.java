...
import sleys.efedp.capability.skills.StacksMultiConditionalWeaponInnateSkill;
...
public class ExtendedDatapacksRegistryStacksConditionalInnateSkills {
    private static final List<String> RUNTIME_ERRORS = Collections.synchronizedList(new ArrayList<>());

    private ExtendedDatapacksRegistryStacksConditionalInnateSkills() {}
    ...
    public static void initialize(IEventBus modBus) {
        ...
        initialized = true;
        ExtendedDatapacks.LOGGER.info("[Stacks Conditional Innate Skills Registry] Registering JSON skills");

        var data = StacksConditionalInnateSkillBuilder.getData();
        if (data.isEmpty()) {
            ExtendedDatapacks.LOGGER.info("[Stacks Conditional Innate Skills Registry] No JSON skills found");
            return;
        }
        ...
        for (var entry : data.entrySet()) {
            ...
            for (var skillData : entry.getValue()) {
                ...
                registry.register(name, key -> {
                    ...
                    var skillBuilder = new StacksMultiConditionalWeaponInnateSkill.Builder(keyBuilder -> new StacksMultiConditionalWeaponInnateSkill(keyBuilder) {
                    ...
                    try {
                        for (var conditionEntry : skillData.animations().entrySet()) {
                            ...
                            StacksConditionalInnateSkillBuilder.ConditionalAnimationData conditionData = conditionEntry.getValue();
                            ...
                            skillBuilder.addConditionData(
                                    type,
                                    new StacksMultiConditionalWeaponInnateSkill.ConditionData(accessor, conditionPhases)
                            );

                            skillBuilder.addStacksData(type, conditionData.stacks());
                        }
    ...
    public static void onClientModBusEvent(final FMLLoadCompleteEvent event) {
        if (!RUNTIME_ERRORS.isEmpty()) {
            ...
            throw new RegistryObjectException(
                    "Failure during the operation to create a Stacks Conditional Innate Skill...\n" +
