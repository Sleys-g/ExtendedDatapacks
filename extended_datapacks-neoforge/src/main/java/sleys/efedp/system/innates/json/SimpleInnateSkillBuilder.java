...
public class SimpleInnateSkillBuilder {
    private static final Map<String, List<RawSimpleInnateSkillBuilderData>> SIMPLE_INNATE_SKILL_BUILD_DATA = new ConcurrentHashMap<>();
    private static final String SL_FOLDER_KEY = "innate_skill_builder/simple_innate_skill";

    public static void startToTracking(Path configDir) {
        SIMPLE_INNATE_SKILL_BUILD_DATA.clear();
    ...
    private static void fileError(String side) {
        ExtendedDatapacks.LOGGER.info(
                "[Simple Innate Skills] There are no parameter files for Simple Innate Skills on the side of {}",
                side
        );
    }
    ...
    private static void startToLoad(Path file, String modId) throws IOException {
        ...
        ExtendedDatapacks.LOGGER.info(
                "[Simple Innate Skills] Reading file: {}",
                file.getFileName()
        );
        startToProcessSimpleInnateSkills(root, file.getFileName().toString(), modId);
    }

    private static void startToProcessSimpleInnateSkills(JsonObject root, String fileName, String modId) {
        if (modId.equals("config")) {

            if (!root.has("modid")) {
                ExtendedDatapacks.LOGGER.warn(
                        "[Simple Innate Skills] missing 'modid' in: {}",
                        fileName
                );
                return;
            }
            ...
            if (root.has("modid")) {
                ExtendedDatapacks.LOGGER.warn(
                        "[Simple Innate Skills] 'modid' ignored in {}, since file is loaded from jar.",
                        fileName
                );
            }
        }
        ...
        if (!root.has("name")) {
            ExtendedDatapacks.LOGGER.warn("[Simple Innate Skills] Skill without 'name' in {}", fileName);
            return;
        }

        if (!root.has("animation")) {
            ExtendedDatapacks.LOGGER.warn("[Simple Innate Skills] Skill without valid 'animation' in {}", fileName);
            return;
        }

        if (!root.has("properties_type")) {
            ExtendedDatapacks.LOGGER.warn("[Simple Innate Skills] Missing 'properties_type' in {}", fileName);
            return;
        }

        if (!root.has("properties")) {
            ExtendedDatapacks.LOGGER.warn("[Simple Innate Skills] Missing 'properties' in {}", fileName);
            return;
        }
        ...
        switch (type) {
            ...
            default -> {
                ExtendedDatapacks.LOGGER.warn("[Simple Innate Skills] Unknown properties_type '{}' in {}", type, fileName);
                return;
            }
        }
        ...
        if (!hasTooltip) {
            ExtendedDatapacks.LOGGER.info("[Simple Innate Skills] The control field for 'tooltip' does not exist, is invalid, or is empty; data behavior is delegated to the standard function in {}...", fileName);
        }
        ...
        RawSimpleInnateSkillBuilderData data = new RawSimpleInnateSkillBuilderData(
                modId, name, animation,
                tooltipData, phases
        );

        SIMPLE_INNATE_SKILL_BUILD_DATA
                .computeIfAbsent(modId, k -> new ArrayList<>())
                .add(data);
    }

    public static Map<String, List<RawSimpleInnateSkillBuilderData>> getData() {
        return SIMPLE_INNATE_SKILL_BUILD_DATA;
    }
